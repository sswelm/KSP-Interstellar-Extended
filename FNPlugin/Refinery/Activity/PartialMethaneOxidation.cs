    ...
    class PartialMethaneOxidation : RefineryActivity, IRefineryActivity
    {
        ...
        public void PrintMissingResources()
        {
            if (!_part.GetConnectedResources(ResourcesConfiguration.Instance.Methane).Any(rs => rs.amount > 0))
                ScreenMessages.PostScreenMessage(Localizer.Format("#LOC_KSPIE_PartialOxiMethane_Postmsg") + " " + ResourcesConfiguration.Instance.Methane, 3.0f, ScreenMessageStyle.UPPER_CENTER);//Missing
            if (!_part.GetConnectedResources(ResourcesConfiguration.Instance.LqdOxygen).Any(rs => rs.amount > 0))
                ScreenMessages.PostScreenMessage(Localizer.Format("#LOC_KSPIE_PartialOxiMethane_Postmsg") + " " + ResourcesConfiguration.Instance.LqdOxygen, 3.0f, ScreenMessageStyle.UPPER_CENTER);//Missing
        }
    }
}

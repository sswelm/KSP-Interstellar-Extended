    ...
    public class ORSPlanetaryResourceMapData : MonoBehaviour
    {
        ...
        protected static GameObject createAbundanceSphere() 
        {
            if (sphere == null) 
            {
                ...
                var collider = resource_prim.GetComponent<Collider>();
                collider.enabled = false;
                resource_prim.transform.localScale = sphere_scale * (FlightGlobals.currentMainBody.Radius / FlightGlobals.Bodies[ORSGameConstants.REF_BODY_KERBIN].Radius);
                var renderer = resource_prim.GetComponent<Renderer>();
                renderer.material.shader = Shader.Find("Unlit/Texture");
                renderer.material.color = new Color(Color.red.r, Color.red.g, Color.red.b, 1.0f);
                
                if (sphere_texture != null)
                    renderer.material.mainTexture = GameDatabase.Instance.GetTexture(sphere_texture, false);
                else
                    renderer.material.mainTexture = GameDatabase.Instance.GetTexture("OpenResourceSystem/resource_point", false);
